    ...
    return (
      <div className="bg-white">
        <div className="max-w-2xl min-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8 ">
          <h2 className="text-2xl font-extrabold tracking-tight text-gray-900">Customers Choice</h2>
  
          <form className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
            {producItem.map((product, index) => (
            ...
            user && arr.map(elem=>{
              if (elem.user.admin) {
                return (
                  <>
                  <span className='absolute z-20 right-2 p-2'><i className="bi bi-three-dots-vertical text-lg"></i></span>
                </>
               )
              }
