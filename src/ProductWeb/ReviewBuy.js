...
function ReviewBuy() {
  let { id } = useParams()
  console.log(id, 'me review') 
  const [pickItem, setpickItem] = useState({ id })
...
// useEffect(() => {
  ...
  return (
    <div>
     {
        data.map(elem => {
          let {color,href,id,imageAlt,imageSrc,name,price}=elem
          return (
            <div className="relative z-10 mt-30" role="dialog" aria-modal="true">
            <div className="hidden fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity md:block"></div>
            <button className='absolute text-red-500 right-0' >X</button>
